...
const TrustSignals: React.FC = () => {
    const signals = [
        {
            ...
            icon: (
                <svg aria-hidden="true" focusable="false" fill="none" strokeWidth="1" width="24" className="w-6 h-6 stroke-1" viewBox="0 0 24 24">
    ...
    return (
        <section className="bg-[#efefef] pt-0 pb-16  font-heading border-tr">
            <div className="max-w-[1440px] mx-auto px-10">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
                    {signals.map((signal, index) => (
                        <div key={index} className="flex flex-col items-center text-center group">
                            <div className="mb-6 text-text opacity-80 group-hover:opacity-100 transition-opacity">
                                {signal.icon}
                            </div>
                            <div className="flex flex-col gap-3">
                                <h3 className="text-[11px] font-medium uppercase tracking-[0.2em] text-text">
                                    {signal.title}
                                </h3>
                                <div className="text-[12px] leading-[1.6] text-[#666] max-w-[280px] mx-auto font-body font-normal">
                                    <p className="m-0 whitespace-pre-line">{signal.text}</p>
                                </div>
                            </div>
                        </div>
                    ))}
